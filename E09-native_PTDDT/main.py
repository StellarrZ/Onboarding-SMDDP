...
def demo(local_world_size, local_rank):
...
def main():
    ...
    # parser.add_argument('--no-ddt', action='store_true', default=True,
    #                     help='disables distributed data parallelism training')
    ...
    rank = dist.get_rank()
    print("Local process %d/%d is running..." %(rank, size))

    if use_cuda and (n_gpus := torch.cuda.device_count()) < 8:
